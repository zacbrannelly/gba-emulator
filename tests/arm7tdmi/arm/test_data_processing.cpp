...
TEST_CASE("Data Processing", "[arm, data-processing]") {
  ...
  SECTION("ADD") {
    ...
    // -5 + 1 = -4
    REQUIRE(cpu.get_register_value(2) == (int32_t)-4);
    REQUIRE(cpu.cpsr & (1 << 31)); // N (Negative) flag should be set.
  }
  ...
  SECTION("ADC") {
    ...
    cpu.set_register_value(2, 0x1);
    cpu.cpsr = (uint32_t)User;
    cpu.cpsr |= 1 << 29; // Set the carry flag.
    cpu_cycle(cpu);
  ...
  SECTION("TST") {
    ...
    // 1 & 1 = 1
    REQUIRE_FALSE(cpu.cpsr & (1 << 30)); // Z (Zero) flag should not be set.
    ...
    // 1 & 0 = 0
    REQUIRE(cpu.cpsr & (1 << 30)); // Z (Zero) flag should be set.
  }
  ...
  SECTION("CMN") {
    ...
    // 1 + 1 = 2
    REQUIRE_FALSE(cpu.cpsr & CPSR_Z); // Z (Zero) flag should not be set.
  ...
  SECTION("MRS") {
    ...
    REQUIRE(cpu.get_register_value(0) == cpu.cpsr);

    // mrs r0, spsr
    cpu.mode_to_scpsr[Supervisor] = 0x12345678;
    cpu_cycle(cpu);
  ...
  SECTION("MSR") {
    ...
    REQUIRE(cpu.mode_to_scpsr[Supervisor] == 0x87654321);
  }
}
